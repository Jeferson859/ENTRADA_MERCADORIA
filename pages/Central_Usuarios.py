    ...
    if usuarios.empty:
        ...
        with c1:
            with st.popover("🔑 Resetar senha", use_container_width=True):
                st.caption("**Opção 1** — senha temporária (o usuário troca no próximo login):")
                if st.button("🎲 Gerar senha temporária", use_container_width=True, key="btn_temp"):
                    with st.spinner("Gravando no GitHub..."):
                        temp = auth.resetar_senha_temporaria(alvo)
                    st.session_state["senha_temp_gerada"] = (alvo, temp)
                gerada = st.session_state.get("senha_temp_gerada")
                if gerada and gerada[0] == alvo:
                    st.success("Copie e envie ao usuário — não será exibida de novo:")
                    st.code(gerada[1])
                st.divider()
                st.caption("**Opção 2** — definir senha manualmente:")
                ns = st.text_input("Nova senha", type="password", key="ns")
                forcar = st.checkbox("Exigir troca no próximo login", value=True, key="ns_forcar")
                if st.button("Salvar nova senha", use_container_width=True):
                    if len(ns) < 6:
                        st.error("Mínimo de 6 caracteres.")
                    else:
                        auth.alterar_senha(alvo, ns, forcar_troca=forcar)
